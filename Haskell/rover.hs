...
nextDirect d "M" = d
nextDirect d "L" = case d of "W" -> "S"
                             "S" -> "E"
                             "E" -> "N"
                             "N" -> "W"
nextDirect d "R" = case d of "W" -> "N"
                             "N" -> "E"
                             "E" -> "S"
                             "S" -> "W"
...
checkResult mc (Just (((x, y), d), _)) | (x, y) `elem` allPos = show x ++ " " ++ show y ++ " " ++ d
                                       | otherwise = "RIP"
...
main :: IO ()
main = do
    let instructs = ["5 5", "1 2 N", "LMLMLMLMM", "3 3 E", "MMRMMRMRRM", "4 4 N", "MMM"]
        mc = getMaxCoordinate $ head instructs
    mapM_ print . map (checkResult mc . runInstructs instructs) $ take (length instructs `div` 2) $ zip [1,3..] [2,4..]
